package com.university.shenyang.air.testing.monitoring.task;
 ...
 */
@Component
public class DayDataTransferTask {
    @Autowired
    ...
    @Value("${day.cal.insertmysql.batch.size:100}")
    private int batchSize;

    @Scheduled(cron = "${day.cal.cron:0 5 0 * * ?}")
    public void dataTransfer() {
        try {
            ...
            long startTime = currentHour.getTime().getTime() - 1000 * 60 * 60 *24;
            //  获取当前时间之前的整点时间的毫秒值
            ...
            if (dayInfoList.size() > 0) {
                if (dayInfoList.size() > batchSize) {
                    for (int i = 0; i < dayInfoList.size() / batchSize; i++) {
                        dayInfoService.batchInsert(dayInfoList.subList(i * batchSize, (i + 1) * batchSize - 1));
                    }
                    if (dayInfoList.size() % batchSize != 0) {
                        dayInfoService.batchInsert(dayInfoList.subList(dayInfoList.size() / batchSize * batchSize, dayInfoList.size() - 1));
                    }
                } else {
                    dayInfoService.batchInsert(dayInfoList);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
